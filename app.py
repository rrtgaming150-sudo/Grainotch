...
TEST_MOBILE = os.environ.get("TEST_MOBILE", "9021993403")  # fallback

if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
    print("⚠️ WARNING: Telegram credentials missing. Telegram alerts disabled.")
...
{Colors.BG_MAGENTA}{Colors.BOLD}{Colors.WHITE}║         {Colors.GREEN}║     SCRIPT CODED BY - CAMPLOOTERS                          ║{Colors.RESET}{Colors.BG_MAGENTA}{Colors.WHITE}      ║{Colors.RESET}
{Colors.BG_MAGENTA}{Colors.BOLD}{Colors.WHITE}║         {Colors.GREEN}║        OWNER - MANMOHAN                                    ║{Colors.RESET}{Colors.BG_MAGENTA}{Colors.WHITE}      ║{Colors.RESET}
{Colors.BG_MAGENTA}{Colors.BOLD}{Colors.WHITE}║         {Colors.GREEN}║   🔗 CHANNEL: https://t.me/CAMPLOOTERS005                  ║{Colors.RESET}{Colors.BG_MAGENTA}{Colors.WHITE}      ║{Colors.RESET}
{Colors.BG_MAGENTA}{Colors.BOLD}{Colors.WHITE}║         {Colors.GREEN}╚════════════════════════════════════════════════════════════╝{Colors.RESET}{Colors.BG_MAGENTA}{Colors.WHITE}      ║{Colors.RESET}
{Colors.BG_MAGENTA}{Colors.BOLD}{Colors.WHITE}╚══════════════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}
    """)

# ================== CONFIGURATION ==================
CHANNEL_LINK = "https://t.me/CAMPLOOTERS005"
...
def generate_code_from_pattern(pattern):
    ...
    for ch in pattern:
        if ch == 'L':
            code += random.choice(ALLOWED_LETTERS)
        else:  # 'D'
            code += random.choice(ALLOWED_DIGITS)
    return code

PATTERN_GENERATORS = [lambda p=p: generate_code_from_pattern(p) for p in PATTERN_STRINGS]

def generate_random_code():
    global CURRENT_PATTERN
    CURRENT_PATTERN = (CURRENT_PATTERN % 3) + 1   # 1..3
    gen_func = PATTERN_GENERATORS[CURRENT_PATTERN - 1]
    for _ in range(100):
        code = gen_func()
...
def send_telegram_start():
    ...
    def send():
        try:
            ...
            pat_list = "\n".join([f"🔹 {i+1}. BMW + {p} (L=Letter, D=Digit)" for i, p in enumerate(PATTERN_STRINGS)])
            message = f"""🚀 BMW CHECKER STARTED!

🔥 CHANNEL: {CHANNEL_LINK}
👑 OWNER: MANMOHAN
💻 CODED BY: CAMPLOOTERS

━━━━━━━━━━━━━━━━━━━━━
📝 3 PATTERNS (L=Letter, D=Digit):
...
📱 Mobile: {TEST_MOBILE}
🔄 Auto Recheck: Every {RECHECK_INTERVAL} batches
━━━━━━━━━━━━━━━━━━━━━"""
...
def process_code(code):
    ...
    with print_lock:
        if result['valid']:
            ...
            print(f"{Colors.MAGENTA}#CAMPLOOTERS{Colors.RESET}\n")
            with open(VALID_CODES_FILE, "a") as f:
...
def main():
    ...
    print(f"{Colors.BOLD}🚀 BMW 3‑PATTERN CHECKER (No Proxy){Colors.RESET}")
    ...
    print(f"\n{Colors.CYAN}📝 3 PATTERNS (L=Letter, D=Digit):{Colors.RESET}")
    for i, p in enumerate(PATTERN_STRINGS, 1):
        print(f"   {Colors.GREEN}{i}. BMW + {p}{Colors.RESET}")
    print(f"{Colors.CYAN}{'='*60}{Colors.RESET}\n")
